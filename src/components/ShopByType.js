      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-2">
        ...
        <div className="grid grid-cols-2  sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-4 w-100% gap-6 sm:gap-8">
          {vehicleTypes.map((type) => (
            ...
            >
              {/* Image wrapper */}
              <div className="relative w-25 h-25 sm:w-25 sm:h-25 md:w-10 md:h-20 lg:w-40 lg:h-25  mb-3 ">
                <Image
                  src={type.image}
                  alt={type.name}
                  fill
              ...
              {/* Name */}
              <span className="text-blue-900 font-medium text-xl lg:text-2xl sm:text-sm md:text-3xl whitespace-nowrap">
                {type.name}
              </span>
            </Link>
          ))}
